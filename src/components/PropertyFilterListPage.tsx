...
interface PropertyFilterListPageProps {
  ...
  title: string;
  /** Italic subtitle shown under the heading */
  description: string;
  /** Message shown when the filtered list is empty */
  emptyMessage: string;
  /**
   * Fetches one page of properties already filtered server-side
   * (e.g. propertiesApi.getProperties bound to isHotSale/isTrending/isExploreNearby).
   */
  fetchProperties: (
    page: number,
    limit: number,
  ) => Promise<{ data: Property[]; meta: PaginationMeta }>;
  /** Optional badge shown next to the Status pill, e.g. "Hot Sale" */
  badgeLabel?: string;
 ...
 * boolean flag) -- used by Hot Sale, Trending and Near By Properties so
 * that structure/search/pagination behavior stays in one place instead of
 * being copy-pasted across near-identical pages.
 */
export function PropertyFilterListPage({
  title,
  description,
  emptyMessage,
  fetchProperties,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error(`Failed to fetch ${title.toLowerCase()} properties:`, error);
    } finally {
      setIsLoading(false);
    }
                ...
                >
                  ...
                  <td className="py-5">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {badgeLabel && (
                        <span
                          className={`text-xs font-medium px-2 py-1 rounded-full ${badgeClassName}`}
                        >
                          {badgeLabel}
                        </span>
                      )}
