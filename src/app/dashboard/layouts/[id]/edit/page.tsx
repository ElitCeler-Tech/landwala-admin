...
export default function EditLayoutPage() {
  ...
  const [originalSlotIds, setOriginalSlotIds] = useState<Set<string>>(
    new Set(),
  );
    ...
    const fetchLayout = async () => {
      try {
        ...
        if (data.slots && data.slots.length > 0) {
          ...
          setOriginalSlotIds(new Set(data.slots.map((s) => s.id)));
        }
      } catch (error) {
  ...
  const addSlot = () => {
    ...
    setNewSlot({
      sectionTitle: newSlot.sectionTitle,
      plotNumber: "",
      area: "",
      facing: "",
      price: "",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1. Update Layout Basic Fields & Images
      await layoutsApi.updateLayout(layoutId, submitData);

      // 2. Handle Slot Updates (Create, Update, Delete)
      const currentSlotIds = new Set(
        slots.filter((s) => s.id).map((s) => s.id as string),
      );

      // Deletions: Slots that were in original but are not in current
      const deletions = Array.from(originalSlotIds).filter(
        (id) => !currentSlotIds.has(id),
      );
      await Promise.all(
        deletions.map((id) => layoutsApi.deleteLayoutSlot(layoutId, id)),
      );

      // Creations: Slots that don't have an ID
      const creations = slots.filter((s) => !s.id);
      await Promise.all(
        creations.map((s) =>
          layoutsApi.createLayoutSlot(layoutId, {
            sectionTitle: s.sectionTitle,
            plotNumber: s.plotNumber,
            area: s.area,
            facing: s.facing,
      ...
      // Updates: Slots that have an ID
      const updates = slots.filter((s) => s.id);
      await Promise.all(
        updates.map((s) =>
          layoutsApi.updateLayoutSlot(layoutId, s.id!, {
            sectionTitle: s.sectionTitle,
            plotNumber: s.plotNumber,
            area: s.area,
            facing: s.facing,
      ...
      setTimeout(() => {
        router.push(`/dashboard/layouts/${layoutId}`);
      }, 2000);
    } catch (err: any) {
      console.error(err);
    ...
    <div className="p-8 max-w-7xl mx-auto pb-12 bg-white font-sans min-h-full flex flex-col">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          <Link
            href={`/dashboard/layouts/${layoutId}`}
        ...
        <p className="text-gray-500 italic ml-8">
          Update the layout details and slots
        </p>
      </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {files.layoutImage ? (
                      ...
                      <span className="text-gray-400">Upload New Map</span>
                    )}
                  </div>
                </div>
              </div>
      ...
      <div className="flex justify-end gap-4 mt-8">
        ...
        <Link href={`/dashboard/layouts/${layoutId}`}>
